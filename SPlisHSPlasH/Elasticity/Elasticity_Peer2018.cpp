...
void Elasticity_Peer2018::computeRHS(VectorXr& rhs)
{
	...
	/*Real cur_time = TimeManager::getCurrent()->getTime();
	if (cur_time < 0.6) {
		m_youngsModulus = 0.0;
	}
	if (cur_time > 0.6 && cur_time < 1) {
		m_youngsModulus = (cur_time - 0.6) / 0.4 * 100000.0;
	}
	if (cur_time > 1) {
...
#pragma omp for schedule(static)  
		for (int i = 0; i < (int)numParticles; i++)
		{
			...
			//Real epsilon_e = sqrt(2 / 9 * ((strain[0] - strain[1]) * (strain[0] - strain[1]) + (strain[1] - strain[2]) * (strain[1] - strain[2])
				//+ (strain[2] - strain[0]) * (strain[2] - strain[0]) + 6 * (strain[3] * strain[3] + strain[4] * strain[4] + strain[5] * strain[5])));
			/*
			Real lefthand = m_youngsModulus / ((1 + mu) * (1 - 2 * mu));
			Real sigma_x = ((1 - mu) * strain[0] + mu * (strain[1] * strain[2])) * lefthand;
			Real sigma_y = ((1 - mu) * strain[1] + mu * (strain[0] * strain[2])) * lefthand;
			Real sigma_z = ((1 - mu) * strain[2] + mu * (strain[0] * strain[1])) * lefthand;
			Real righthand = m_youngsModulus / (2 * (1 + mu));
			Real tau_xy = strain[3] * righthand;
			Real tau_yz = strain[5] * righthand;
			Real tau_zx = strain[4] * righthand;

			std::cout << m_youngsModulus << std::endl;
			*/
			...
			Real tau_xy = m_stress[i][3];
			Real tau_yz = m_stress[i][4];
			Real tau_zx = m_stress[i][5];
